...
def archivio_db(tmp_path):
    ...
    _mail(conn, "m8", "fi@muster.ch", sig_vollstaendig.replace("anna@beispiel.ch", "fi@muster.ch"), "2026-01-01")
    _mail(conn, "m9", "fi@muster.ch", sig_vollstaendig.replace("anna@beispiel.ch", "fi@muster.ch"), "2026-01-02")
...
def test_automatisierte_systemadressen_werden_nie_kandidat(tmp_path, tmp_db):
    ...
    conn = _neue_signatur_db(pfad)
    sig = "Hans Muster\nMuster AG\nMusterweg 1\n8000 Zuerich\nT 044 111 22 33\nhans@muster.ch"
    _mail(conn, "m1", "no-reply@beispiel.ch", sig, "2026-01-01")
    _mail(conn, "m2", "no-reply@beispiel.ch", sig, "2026-01-02")
    conn.commit()
    conn.close()
...
def test_titelname_wird_durch_email_und_textverifikation_korrigiert(tmp_path, tmp_db):
    ...
    Mailadresse h.muster@beispiel.ch abgeleitet + im Volltext verifiziert soll der
    ...
    text = (
        ...
        "Freundliche Gruesse\n\nHans Muster\nProjektleitung Systeme\nBeispiel Logistics AG\n"
        "Wangentalstrasse 252, 3173 Oberwangen\nT +41 31 111 11 77\nH.Muster@beispiel.ch"
    )
    _mail(conn, "m1", "H.Muster@beispiel.ch", text, "2026-01-01")
    _mail(conn, "m2", "H.Muster@beispiel.ch", text, "2026-01-02")
    conn.commit()
    conn.close()

    kandidaten = hole_kandidaten(str(pfad), tmp_db, min_mails=2)
    assert len(kandidaten) == 1
    assert kandidaten[0]["vorname"] == "Hans"
    assert kandidaten[0]["nachname"] == "Muster"
...
def test_ist_plausibler_personenname():
    ...
    assert _ist_plausibler_personenname("EINWOHNERGEMEINDE", "MUSTERHAUSEN") is False
    assert _ist_plausibler_personenname("", "") is False


def test_name_aus_email():
    from archivio_bridge.anbindung import _name_aus_email
    assert _name_aus_email("h.muster@beispiel.ch") == ("H", "Muster")
...
def test_name_bleibt_unveraendert_wenn_keine_ableitung_aus_email_moeglich(tmp_path, tmp_db):
    ...
    _mail(conn, "m1", "info@beispiel.ch", text, "2026-01-01")
    _mail(conn, "m2", "info@beispiel.ch", text, "2026-01-02")
    conn.commit()
    conn.close()

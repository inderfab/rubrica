...
Tel. +41 52 111 11 11
Mobile +41 79 111 11 22
anna.muster@muster.ch
www.muster.ch
"""
...
def test_email_und_web():
    ...
    assert {"typ": "Direkt", "email": "anna.muster@muster.ch"} in d["emails"]
    assert any(u["url"] == "www.muster.ch" for u in d["urls"])
...
def test_firma_und_name():
    d = parse_signatur(SIG_TYPISCH)
    assert d["firma"] == "Muster Architektur AG"
    assert d["vorname"] == "Anna"
    assert d["nachname"] == "Muster"
...
def test_literale_br_tags_werden_als_zeilenumbruch_behandelt():
    ...
    # Namen und verunstaltet ihn ("Marcel Muster<br>").
    text = "Freundliche Gruesse<br>Marcel Muster<br>Architekt FH<br>Muster Architektur AG"
...
def test_adresse():
    ...
    a = d["adressen"][0]
    assert a["strasse"] == "Musterstrasse 1"
    assert a["plz"] == "8000"
    assert a["ort"] == "Zürich"
...
def test_newsletter_absatz_wird_nicht_als_firma_uebernommen():
    absatz = ("Sie erhalten diese E-Mail, weil Sie bei uns als Kunde hinterlegt und als "
              "Newsletter-Abonnent eingetragen sind. Um Sie ueber die Neuigkeiten der Beispiel AG "
...
def test_gueltige_ch_nummern_bleiben_erhalten():
    ...
    assert any("52 111 11 33" in n for n in nummern)
    assert any("44 111 11 44" in n for n in nummern)

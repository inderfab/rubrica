...
# Testdatensatz beobachtet: "no-reply@beispiel.ch" lieferte einen Kandidaten mit den
...
# Zürich", "EINWOHNERGEMEINDE MUSTERHAUSEN").
...
def _ist_plausibler_personenname(vorname: str, nachname: str) -> bool:
    ...
    if text.isupper() and len(text.split()) > 1:
        ...
        # Organisationsname (z.B. "EINWOHNERGEMEINDE MUSTERHAUSEN"), waehrend ein
...
def _name_aus_email(absender_email: str) -> tuple:
    ...
    'h.muster@...' -> ('H', 'Muster')). Liefert ('', '') wenn das Muster nicht
...
def _name_im_text_verifizieren(text: str, nachname: str):
    ...
    'Freundliche Gruesse Hans Muster') - liefert die im Text tatsaechlich
...
def _verbessere_namenerkennung(daten: dict, absender_email: str, text: str) -> None:
    ...
    H.Muster@beispiel.ch wurde "Projektleitung Systeme" statt eines echten Namens
    erkannt, obwohl im Text vermutlich "... Gruesse Hans Muster" o.ae. steht."""

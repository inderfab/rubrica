...
Entwicklung erfolgt auf dem Mac Studio (Benutzer `anna`), das fertige Produkt läuft produktiv auf einem iMac im Büro.
...
Claude Code sollte sich für den Paketierungs-Ansatz (pkg-Build, launchd-Plists, Bibliothekspfade) am bestehenden Code von Archivio orientieren, der unter `/Users/dev/archivio` liegt, damit beide Tools demselben Muster folgen.
...
- Dieser Teil ist bewusst kein Bestandteil der ersten Umsetzung, sondern wird erst angegangen, wenn Phase 1–2 stehen. Der bestehende Archivio-Code liegt unter `/Users/dev/archivio` und sollte zu gegebener Zeit als Referenz für das tatsächliche SQL-Schema herangezogen werden, bevor die Extraktions- und Matching-Logik entworfen wird.
...
- ~~macOS Kontakte.app synchronisiert nach behobenem TLS zwar fehlerfrei die Discovery, startet aber den
  ...
  - **Lösung:** (1) Account entfernen, Kontakte.app beenden, `killall contactsd`; (2) alle leeren,
    ...
    Hostnamen** (`Anna-Mac-Studio.local`), **ohne Port und ohne Pfad** — macOS macht dann die
...
Flache Struktur analog zu Archivio (`/Users/dev/archivio`) statt einer `backend/frontend/packaging`-Aufteilung – gleiches, bewährtes Muster für Config-Loading, DB-Connection und spätere `.pkg`-Paketierung, keine doppelten Lösungswege für dasselbe Problem.
...
- **Test-`.pkg` mit launchd + Zertifikat (2026-07-10):** `scripts/build-pkg.sh` baut `Rubrica Server.app`
  ...
  Fallback-Pfad) und ein `.pkg`, das zwei launchd-Dienste einrichtet (`ch.rubrica.server`,
  `ch.rubrica.radicale`), beim Erststart automatisch ein selbstsigniertes Zertifikat sowie ein
...
- **Archivio-Anbindung, erste Stufe (2026-07-12):** Neues Modul `archivio_bridge/anbindung.py` (bewusst
  nicht `archivio` genannt, um Verwechslung mit dem Referenzprojekt `/Users/dev/archivio` zu vermeiden) liest
...
- **Menubar-App ersetzt zwei separate launchd-Dienste (2026-07-12):** Bisher liefen Web-Server und Radicale
  als zwei unabhaengige launchd-Dienste (`ch.rubrica.server`, `ch.rubrica.radicale`) ohne jede
  ...
  einen LaunchAgent und raeumt den alten `ch.rubrica.radicale`-Agent von frueheren Installationen auf
...
- **Einstellungsseite (2026-07-12):** Nutzer-Feedback: `archivio.db_path` liess sich nicht setzen, weil die
  ...
  `archivio.db_path` auf den echten lokalen Pfad (`/Users/dev/archivio/archivio.db`) gesetzt — Archivio-Vorschau
...
Archivio (anderes, produktiv laufendes Projekt unter `/Users/dev/archivio`) selbst, separat von dieser
...
- **Archivio-Signatur-DB + Postfach-Features umgesetzt (2026-07-14):** Nutzer hat Archivio umgebaut und einen
  echten Testdatensatz geliefert (10'456 Mails, `/Users/dev/Desktop/rubrica db test/rubrica.db`). Tatsaechliches
  ...
  ```
  `postfach` (Slug, z.B. `200_Keller_Zürich`) und `projekt` (Anzeigename, z.B. "200 Keller Zürich")
  ...
  - **Echter Architektur-Fund beim Pruefen der echten Daten:** Archivio liefert jetzt den VOLLEN Thread-Text
    ...
    werden.
  - **Weiterer Fund:** 36% der Testdaten (3814/10456) waren von der eigenen `@muster.ch`-Domain - neue
    `EIGENE_DOMAIN`-Konstante filtert eigene Mitarbeiter vor der Signatursuche komplett aus (nie ein
    Import-Kandidat). Ausserdem ein no-reply@beispiel.ch (Plot-/Druckauftrags-Benachrichtigung) lieferte einen
...
- **Archivio-Import: bessere Namenserkennung + Bearbeiten-Button (2026-07-14):** Nutzer testete den obigen
  ...
  (Beispiel: "Projektleitung Systeme" bei `H.Muster@beispiel.ch`, wo der echte Name im Text stand). Vorschlag
  ...
  - **Namens-Plausibilitaetspruefung** (`_ist_plausibler_personenname`): verwirft Funktions-/Organisations-
    ...
    Mehrwort-Zeilen (Organisationen wie "EINWOHNERGEMEINDE MUSTERHAUSEN" - ein Nachname wie "Roland
    GUNZENHAUSER" bleibt unberuehrt, da "Roland" gemischt geschrieben ist).
  - **`_name_aus_email`**: leitet Vor-/Nachname aus dem Mail-Lokalteil ab (z.B. `h.muster@...` ->
    `Muster`), verwirft generische Lokalteile (`info@`, `kontakt@` etc.).
  - **`_name_im_text_verifizieren`**: sucht im Volltext nach "Vorname Nachname" mit dem abgeleiteten
    Nachnamen (z.B. findet "... Gruesse Hans-Ulrich Muster" im Text) - liefert die im Text tatsaechlich
  ...
  - **Nebenfund:** eine informelle Anrede am Textanfang ("Hoi Marcel") wurde faelschlich als Absender-Name
    ...
    "Marcel Muster<br>") - `parse_signatur` ersetzt `<br>`/`<br/>` jetzt durch echte Zeilenumbrueche.
...
- **Menubar-Icon (2026-07-13):** Bisher nur ein Emoji ("📇") als Menubar-Titel. Nutzer hat ein eigenes
  ...
  (`/Users/dev/archivio/menubar/icon.png`): Logo auf 44×44 skaliert nach `menubar/icon.png`, `RubricaApp`
...
- **PDF-Export: Firmenzeile getrennt von Mitarbeiterzeilen, Sichtbarkeits-Einstellungen, echte Trennlinien
  ...
  - Ausserdem: Platzhaltertext beim Firmennamen-Feld war "Muster Architektur AG" (Nutzer: soll bei anderen
...
- **Telefon-/E-Mail-Kategorien vereinheitlicht auf Direkt/Privat/Allgemein, Export-Kopf/Fuss verfeinert
  ...
  - Live-Test des Nutzers zeigte ausserdem einen Kontakt ("Manon Mathys") auf einer separaten Zeile statt im
    selben Firmenblock wie die uebrigen Muster-Mitarbeiter - Ursache war ein Tippfehler im Firmenfeld dieses
    einen Kontakts ("Muster Architeken AG" statt "Muster Architektur AG"); die Gruppierung ist bewusst ein
